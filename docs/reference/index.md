---
title: Reference
nav_order: 20
has_children: true
---

# Reference Documentation

This section is the implementation-level map of packages, classes, and responsibilities.

## Layer References

- [Interface HTTP Layer](./api-layer.md)
- [Application Layer](./application-layer.md)
- [Domain Layer](./domain-layer.md)
- [Infrastructure Layer](./infrastructure-layer.md)
- [Configuration and Runtime Controls](./configuration-and-runtime.md)

## How to Use This Section

- Start with the layer page matching your change scope.
- Use package and class listings to locate integration points quickly.
- Cross-check runtime properties in configuration reference before behavior changes.
